...
const TourDetail = () => {
  ...
  useEffect(() => {
    const getSingleTours = async () => {
        debugger
        try {
            const response = await customAxios.get('tours/getSingle/${ids}');
            let result = response.data;
            setSingletour(result);
        } catch (error) {
            console.log(error)
        }
    };
...
//   const getSingleTour = (ids) => {
//     debugger;
//     customAxios
//       .get(`tours/getSingle/${ids}`)
//       .then((response) => {
//         let result = response.data;
//         setSingletour(result);
//       })
    ...
    <>
      <Container>
        <Row>
                      ...
                      <div className="w-100">
                        <div className="d-flex align-items-center justify-content-between">
                          <div>
                            <h5>anees</h5>
                            <p>
                              {new Date("11-21-2023").toLocaleDateString(
                                "en-US",
                                option
                              )}
                            </p>
          ...
          <Col lg="4">
            <Booking tour={singletour} avgRating={avgRating} />
          </Col>
        </Row>
      </Container>
      <NewsLetter />
